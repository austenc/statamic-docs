...
for url in sorted(set(links)):
    ...
    if ('css' in url or 'img' in url):
        continue

    # topic = url.split('/')[3].title().replace('-', ' ').split('#')[0]
    # slug = url.split('/')[3].split('#')[0]
    topic = url.title().replace('-', ' ')
    slug = url

    if (topic not in found and topic != ""):
        found.append(topic)
        pages.append({
            "topic": topic,
            "slug": slug,
            "command": "statamicDocs" + slug.replace('-', ' ').title().replace(' ', '')
        })
...
with open(os.path.join(root, '../src/extension.ts'), 'w+') as f:
    ...
    f.write('    let version = vscode.workspace.getConfiguration("statamicDocs").version;' + "\n")
    f.write(
